...
test.describe('admin sidebar (T11.1)', () => {
  ...
  test('visual: expanded sidebar (STARTER)', async ({ page }, testInfo) => {
    ...
    await expect(sidebar).toHaveScreenshot(`sidebar-expanded-${testInfo.project.name}.png`, {
  ...
  test('visual: collapsed sidebar (STARTER)', async ({ page }, testInfo) => {
    ...
    await expect(sidebar).toHaveScreenshot(`sidebar-collapsed-${testInfo.project.name}.png`, {
      maxDiffPixelRatio: 0.05,
    });
  });
  ...
  test('functional: nav item click navigates and active state flips', async ({ page }) => {
    ...
    await expect(page.getByTestId('sidebar-nav-dashboard')).toHaveAttribute('data-active', 'true');

    await page.getByTestId('sidebar-nav-menus').click();
    await expect(page).toHaveURL(/\/admin\/menus(\b|\/)/);

    await expect(page.getByTestId('sidebar-nav-menus')).toHaveAttribute('data-active', 'true');
  ...
  test('functional: collapse toggle persists via localStorage across reloads', async ({ page }) => {
    await seedAndLogin('STARTER', page);
    await page.goto('/admin/dashboard');
    ...
    const stored = await page.evaluate(() => localStorage.getItem('dm.sidebar.collapsed'));
    ...
    const storedAfter = await page.evaluate(() => localStorage.getItem('dm.sidebar.collapsed'));
    expect(storedAfter).toBe('false');
  });

  test('functional: Upgrade CTA hidden on PRO, visible on FREE and STARTER', async ({ page }) => {
    await seedAndLogin('PRO', page);
    await page.goto('/admin/dashboard');
    await expect(page.getByTestId('sidebar-upgrade-cta')).toHaveCount(0);
  ...
  test('functional: menu switcher lists real menus for the signed-in user', async ({ page }) => {
    const email = 'real-menus@cafelinville.ge';
    ...
    const menu = await seedMenu({
      userId: user.id,
      name: 'Rustaveli Breakfast',
      slug: 'rustaveli-breakfast',
      status: 'PUBLISHED',
      categoryCount: 1,
      productCount: 1,
    });
    ...
    await expect(page.getByTestId('sidebar-org-switcher')).toContainText('Rustaveli Breakfast');
    await expect(page.getByTestId('sidebar-org-switcher')).toContainText('/m/rustaveli-breakfast');

    await page.getByTestId('sidebar-org-switcher').click();
    const switcher = page.getByTestId('sidebar-menu-switcher');
    await expect(switcher).toContainText('Rustaveli Breakfast');
    await expect(switcher).toContainText('/m/rustaveli-breakfast');

    await page.getByTestId(`sidebar-menu-item-${menu.id}`).click();
    await expect(page).toHaveURL(new RegExp(`/admin/menus/${menu.id}$`));
  });

  test('functional: selected menu becomes the persisted sidebar default', async ({ page }) => {
    const email = 'default-menu@cafelinville.ge';
    const user = await seedUser({
      plan: 'STARTER',
      name: 'Nino Kapanadze',
      email,
    });
    const lunch = await seedMenu({
      userId: user.id,
      name: 'Lunch Menu',
      slug: 'lunch-menu',
      status: 'PUBLISHED',
    ...
    await page.goto('/admin/dashboard');
    await page.getByTestId('sidebar-org-switcher').click();
    await page.getByTestId(`sidebar-menu-item-${lunch.id}`).click();
    await expect(page).toHaveURL(new RegExp(`/admin/menus/${lunch.id}$`));

    await page.goto('/admin/dashboard');
    await expect(page.getByTestId('sidebar-org-switcher')).toContainText('Lunch Menu');

    const stored = await page.evaluate(
      (key) => localStorage.getItem(key),
      'dm.sidebar.defaultMenu.default-menu@cafelinville.ge'
    );
    expect(stored).toBe(lunch.id);
  });
});

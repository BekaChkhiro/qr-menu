...
- [x] **Status**: ✅ DONE · **Dependencies**: T22.7, T22.18
...
- [x] **Status**: ✅ DONE · **Dependencies**: T22.3, T22.19
...
- [x] **Status**: ✅ DONE · **Dependencies**: T22.6
...
- [x] **Status**: ✅ DONE · **Dependencies**: T22.20, T22.9
...
- [x] **Status**: ✅ DONE · **Dependencies**: T22.18
- **Complexity**: High
...
- [x] **Status**: ✅ DONE · **Dependencies**: T22.19
- **Complexity**: High
...
- [x] **Status**: ✅ DONE · **Dependencies**: T22.22
...
### Phase 24: Promotions V2 — Owner Report Follow-ups

**Scope**: The 21 Promotions V2 owner-report follow-ups, renumbered one-to-one from the former promotion comment IDs; Phase 23 remains reserved for Analytics. Paths below are relative to `apps/web/` unless prefixed with `tests/e2e/` or `packages/`.

**Audit (2026-09-23)**: DONE means the behavior is implemented and has been checked; missing dedicated regression coverage is noted separately and does not by itself make a task PARTIAL. The functional and existing-flow gaps in T24.4, T24.8, T24.14, and T24.20 are now resolved; all 21 tasks are DONE. Verification: `pnpm --filter web exec tsc --noEmit` passed after dependencies were restored; targeted Vitest passed 45/45 tests across the date, pricing, product-rail, menu-body-jump, product-form-language, and category-API suites. Referenced Playwright browser/DB tests were inspected but not run locally; missing regression coverage is called out per task.

#### T24.1: Optional Promotion Validity Dates
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.21, T22.25
- **Description**: `packages/database/prisma/schema.prisma`, `lib/validations/promotion.ts`, and promotion create/update routes support either or both date boundaries being absent, with null clearing a stored date and an omitted update preserving it. `promotions-list.tsx` displays an em dash for missing dates, `editor-promotions-tab.tsx` uses localized open-ended labels, and `promotion-calendar.tsx` extends missing bounds to the visible grid; undated promotions follow their active switch.
- **Verification**: Matching comments and nullable implementation exist in those files; `lib/promotions/__tests__/time-windows-dates.test.ts` asserts no-date, start-only, and end-only evaluation. Dated query freshness/timezone gaps are tracked under T24.14.

#### T24.2: Toggle Promotions Directly from Cards
- [x] **Status**: ✅ DONE
- **Dependencies**: T15.7, T24.1
- **Description**: `components/admin/editor-promotions-tab.tsx` renders an on/off switch on each promotion card and calls `useTogglePromotionActive` in `hooks/use-promotions.ts`. The hook sends only `isActive` to the update endpoint and refreshes promotion/menu queries.
- **Verification**: Comment, switch handler, mutation, and update route were checked; `tests/e2e/admin/editor-promotions.spec.ts` covers filters and cards. No dedicated regression test yet: card toggle persistence and public visibility.

#### T24.3: Default-On Weekdays and Explicit Off State
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.21, T24.4
- **Description**: `components/admin/day-windows-editor.tsx` supplies all seven 09:00–18:00 windows when venue hours are unavailable, otherwise prefills open venue days and leaves closed days off. Promotion/product restriction toggles use this default; active rows use the success border and disabled rows use the danger border/background.
- **Verification**: Comments and default/style handlers were checked in `day-windows-editor.tsx`, `promotion-drawer.tsx`, and `product-form.tsx`; `tests/e2e/public/promotion-time-gating.spec.ts` covers price gating. No dedicated regression test yet: weekday defaults, closed-day exceptions, and on/off styling.

#### T24.4: Venue Working Hours and Optional Break
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.21, T22.23
- **Description**: `components/admin/menu-working-hours-section.tsx` saves weekday opening/closing hours and optional breaks, `lib/menu/working-hours.ts` normalizes/groups them, and `components/public/menu-footer.tsx` renders the schedule. The shared day-window editor prefills from those hours, while `lib/promotions/time-windows.ts` excludes configured breaks from active windows.
- **Verification**: Disabling `menu-working-hours-section.tsx` saves null; `lib/validations/menu.ts` accepts nullish hours and `app/api/menus/[id]/route.ts` persists `Prisma.DbNull`. `lib/menu/working-hours.ts` normalizes legacy all-closed arrays to null. `lib/menu/__tests__/working-hours.test.ts` covers disabled/legacy states, omitted updates, and break windows.

#### T24.5: No Category Image Means No Category Banner
- [x] **Status**: ✅ DONE
- **Dependencies**: T21.11
- **Description**: `components/public/category-section.tsx` renders a plain category name and product count when `iconUrl` is absent, with no generated banner or placeholder image. Categories with artwork retain their image banner.
- **Verification**: Matching comments and the `!category.iconUrl` branch exist; `tests/e2e/public/category-section-header.spec.ts` asserts the plain header, absence of banner/image, and retains the image-banner and visual-baseline cases (E2E not executed locally).

#### T24.6: Promotion Card Thumbnail Uses Authored Appearance
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.20, T24.21
- **Description**: `PromoBanner` in `components/admin/editor-promotions-tab.tsx` renders the promotion's actual image, chosen background color, or neutral fallback instead of an unrelated generated thumbnail. The authored show-title setting controls its title overlay.
- **Verification**: Authored image/color branches and `data-banner-source` were checked; `tests/e2e/admin/editor-promotions.spec.ts` includes a general list screenshot. No dedicated regression test yet: authored thumbnail and show-title parity.

#### T24.7: Duplicate Promotion with Localized Copy Suffix
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.1, T22.24
- **Description**: `app/api/menus/[id]/promotions/[pid]/duplicate/route.ts` checks ownership and the promotions plan feature, copies authored fields, appends localized title suffixes, places the copy last, and starts it inactive. `syncComboProduct` creates a separate combo product when needed; `hooks/use-promotions.ts` and the promotion-card kebab wire up the action.
- **Verification**: Comments and the endpoint, hook, and UI implementation were checked. No dedicated regression test yet: copied fields/suffixes, inactive/order defaults, ownership/plan rejection, and independent combo products.

#### T24.8: Keep the System Offers Category Last
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.24
- **Description**: `app/api/menus/[id]/categories/route.ts` excludes the system Offers category when counting regular category positions, then calls `pushOffersCategoryLast` in `lib/promotions/combo.ts`. Combo synchronization also re-pins Offers after the last regular category.
- **Verification**: Category creation and `app/api/menus/[id]/categories/reorder/route.ts` call `pushOffersCategoryLast`. `tests/api/categories.test.ts` covers the excluded count, creation position, and Offers-last enforcement after reorder before response/broadcast.

#### T24.9: Carry Combo Artwork to the Generated Product
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.24
- **Description**: `syncComboProduct` in `lib/promotions/combo.ts` selects the promotion image and copies `imageUrl` into both creation and update of the generated Offers product. This gives the combo product card the same authored artwork as its promotion.
- **Verification**: The shared artwork create/update data was checked; the combo case in `tests/e2e/admin/editor-promotions-drawer.spec.ts` asserts category/name/price. No dedicated regression test yet: combo image creation, replacement, and removal.

#### T24.10: Preserve Category Artwork when Cloning Menus
- [x] **Status**: ✅ DONE
- **Dependencies**: T15.15, T21.11
- **Description**: `app/api/menus/[id]/clone/route.ts` copies each category's `iconUrl` and `brandLabel` into the cloned menu, alongside its translated names/descriptions. Cloned categories therefore retain their banner artwork and label.
- **Verification**: The copied artwork/label fields were checked; `tests/e2e/admin/editor-settings-advanced.spec.ts` covers menu cloning. No dedicated regression test yet: category artwork/label preservation and absent artwork.

#### T24.11: Move Existing Price into Original on Discount Enable
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.23
- **Description**: `handleDiscountToggle` in `components/admin/product-form.tsx` moves a populated price into an empty original-price field and clears the sale input when enabling a discount. Disabling restores the original only when the sale input is still empty, then clears discount metadata.
- **Verification**: Price transfer/restore branches were checked; `tests/e2e/admin/product-drawer-basics.spec.ts` covers manually entered discount prices. No dedicated regression test yet: automatic transfer/restore and already-discounted prices.

#### T24.12: Hide Language Switchers on Georgian-Only Menus
- [x] **Status**: ✅ DONE
- **Dependencies**: T21.8
- **Description**: `components/admin/product-drawer/lang-tabs-inline.tsx` derives available languages from the menu, always includes KA, hides a one-language strip, and locks enabled EN/RU tabs when the plan lacks multilingual support. `product-dialog.tsx` and `promotion-drawer.tsx` pass the menu's actual enabled languages into that control.
- **Verification**: Language filtering and conditional rendering were checked; `tests/e2e/admin/product-form-lang-sync.spec.ts` covers multilingual menus and plan locks. No dedicated regression test yet: KA-only and partially enabled languages in both drawers.

#### T24.13: Promotion Auto-Advance Scrolls Only the Track
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.22
- **Description**: `centerSlide` in `components/public/promotion-carousel.tsx` calculates a bounded horizontal offset from track/slide rectangles and calls `track.scrollTo`. Automatic carousel movement therefore avoids `scrollIntoView`, which could pull the whole page back to the banner.
- **Verification**: The track-only scrolling implementation was checked; `tests/e2e/public/promotion-carousel-detail.spec.ts` covers details opening/closing. No dedicated regression test yet: unchanged page scroll position during timer-driven track advance.

#### T24.14: Evaluate Dates as Whole Café-Local Days
- [x] **Status**: ✅ DONE
- **Dependencies**: T24.1, T22.21
- **Description**: `lib/promotions/time-windows.ts` provides inclusive local-day bounds for `isScheduled`, `hasEnded`, and Prisma date filters; `lib/public-menu.ts` reevaluates fetched promotions using the menu timezone. `lib/promotions/__tests__/time-windows-dates.test.ts` asserts Tbilisi start/end days, midnight rollover, open boundaries, and invalid-timezone fallback.
- **Verification**: The static `liveDateWhere()` query filter was removed from `lib/public-menu.ts`; public route and preview evaluate dates and hours at runtime using the menu timezone, and promotion create/update API activity checks pass `menu.timezone`. `lib/promotions/__tests__/time-windows-dates.test.ts` covers local-day boundaries, long-running queries, and a non-default timezone.

#### T24.15: Strongest Applicable Promotion Wins
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.18, T22.23
- **Description**: `applyPromotionPricing` in `lib/public-menu.ts` compares final prices from menu-wide and matching-category offers and chooses the lowest, while preserving an existing manual dish discount. Thus menu-wide 5% loses to category 10% but beats category 3%; this describes the current implementation rather than the older addendum's last-applied wording.
- **Verification**: The report comment and eight regression cases in `lib/__tests__/promotion-pricing.test.ts` cover stronger/weaker category offers, scope isolation, manual overrides, fixed/percentage competition, non-price promotions, and expiry.

#### T24.16: Consistent 16:9 Banner Upload and Crop
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.22
- **Description**: `lib/cloudinary/index.ts` uses a 1200×675 promotion preset, matching the 16:9 public/admin banner surfaces. `components/admin/promotion-drawer.tsx` enables the existing cropper for its video-aspect upload field so the operator can frame the image.
- **Verification**: Preset dimensions and cropper props were checked; `tests/e2e/admin/promotion-upload-async.spec.ts` covers upload progress with a mocked PNG response. No dedicated regression test yet: 16:9 transform/crop confirmation and retained framing.

#### T24.17: Preserve GIF/JPEG/WebP MIME Types on Upload
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.22, T24.16
- **Description**: `app/api/upload/route.ts` passes the uploaded file's MIME type into `uploadImage` in `lib/cloudinary/index.ts`. Buffer uploads use that MIME in their data URI instead of labeling every file as PNG.
- **Verification**: MIME propagation was checked; existing promotion-upload tests cover a mocked PNG upload. No dedicated regression test yet: GIF/JPEG/WebP MIME propagation and animated-GIF round trip.

#### T24.18: Offers Rail and See-All Category Navigation
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.24
- **Description**: `components/public/offers-carousel.tsx` shares `product-rail.tsx` with Most Ordered and mirrors Offers above the body on both public menu routes. Its see-all action uses `jump-to-category.ts` and `menu-body.tsx` to reveal the target even from the category grid or an incompatible food/drink filter, then scroll there.
- **Verification**: Matching comments and both route integrations exist; `components/public/__tests__/product-rail.test.tsx` and `menu-body-jump.test.tsx` cover localized heading, empty state, discount badge, event/fallback scrolling, grid exit, filter reset, and unknown targets.

#### T24.19: Per-Menu Most Ordered Visibility Toggle
- [x] **Status**: ✅ DONE
- **Dependencies**: T13.6, T24.18
- **Description**: `Menu.featuredEnabled` defaults to true in `packages/database/prisma/schema.prisma`, is validated/serialized, and is saved by `components/admin/content-display-card.tsx`. Both public menu routes conditionally render Most Ordered while preserving the default-on behavior for legacy menus.
- **Verification**: Schema/validation, save handler, and both route guards were checked; `product-rail.test.tsx` covers FeaturedCarousel. No dedicated regression test yet: persisted default/on/off states across both public routes.

#### T24.20: Bind Translated Inputs to the Active Language
- [x] **Status**: ✅ DONE
- **Dependencies**: T21.8, T24.12
- **Description**: `components/admin/product-form.tsx` and `promotion-drawer.tsx` read the active name/title/description key with `watch` and write it with `setValue`; their language strip sits directly above those fields. `components/admin/__tests__/product-form-lang-switch.test.tsx` asserts independent name/description round trips and loading an existing translation.
- **Verification**: The watch/setValue bindings and `components/admin/__tests__/product-form-lang-switch.test.tsx` cover translated inputs. `tests/e2e/admin/product-form-lang-sync.spec.ts` now switches language in Basics before checking Variations (E2E not executed locally). No dedicated regression test yet: promotion edit/save round trip.

#### T24.21: Neutral Fallback for Unstyled Title-Only Banners
- [x] **Status**: ✅ DONE
- **Dependencies**: T22.20, T24.6
- **Description**: The public `promotion-carousel.tsx` and admin `editor-promotions-tab.tsx` / `promotion-drawer.tsx` use `hsl(var(--muted))`, `ring-1 ring-border`, and `text-foreground` when neither artwork nor an operator-selected color exists. Explicit colors retain white title treatment; image behavior remains authored, and the unrelated calendar chip palette is unchanged.
- **Verification**: The banner thread implemented and checked the neutral fallback across all three surfaces; `tests/e2e/public/promotion-variants.spec.ts` covers an explicit background color. No dedicated regression test yet: neutral fallback in public, admin card, and drawer preview.
...
- **Phase 22 - UX Refinement (Promotions/QR/Settings/Public)**: 0/17 (0%) — plus the Promotions & Discounts addendum (T22.18–T22.25): 8/8 (100%) ✅ shipped
- **Phase 23 - Analytics Tab Deep Review**: 0/5 (0%) — 4 tasks blocked on T23.1 discovery audit
- **Phase 24 - Promotions V2 Owner Report**: 21/21 (100%) ✅ — regression coverage gaps noted separately

### Current Focus
🎯 **Status**: Phase 24 audited — 21 DONE; documented functional and existing-flow fixes completed. Overall 103/184 (56%); existing phase counts retained as previously recorded. Phase 23 Analytics remains unchanged.
...
Last Updated: 2026-09-23 (Phase 24 Promotions V2 Owner Report added: 17/21 DONE, 4 PARTIAL; promotion references renumbered to T24; Phase 23 Analytics preserved; overall 99/184 = 54%).

...
**Created**: 2026-01-26
**Last Updated**: 2026-04-24 (T14.3 done ✅ — Product Drawer · Variations Tab shipped end-to-end. New `product-drawer-variations-tab.tsx` renders the design's 5-col grid (drag handle · Name · Price modifier ±N₾ · Default radio · Kebab) on top of dnd-kit with `useSortable` + PointerSensor(distance:8) + KeyboardSensor; inline "+ Add variation" dashed CTA expands an in-table row with autoFocus KA-name Input + tabular-nums modifier Input (absolute price = base + modifier, clamped to (0, 100000)); Default radio is a role="radio" pill that triggers PUT /variations/[vid] {isDefault:true}; kebab → AlertDialog-gated DELETE. Prisma `ProductVariation.isDefault Boolean @default(false)` added and pushed to Neon (additive); POST/PUT API routes now run a transaction that demotes any existing default before promoting the target so the "exactly 0 or 1 default per product" invariant holds. `createProductVariationSchema` + `updateProductVariationSchema` + `ProductVariation` TS type extended with `isDefault`. `ProductDialog` gained a required `menuId` prop and swaps the placeholder for `<ProductDrawerVariationsTab>` only in edit mode (create mode shows a "Save the product first" placeholder since variations need the parent record). `products-list.tsx` passes `menuId` through to both dialog instances. EN/KA/RU `admin.products.drawer.variationsTab.*` keys added + `placeholders.variationsCreate.{title,body}` for the create-mode fallback. New Playwright spec `tests/e2e/admin/product-drawer-variations.spec.ts` (serial, 10 tests listed = 5 desktop + 5 mobile-skipped) — 1 visual baseline `product-drawer-variations-desktop.png` (STARTER Nino seed + 3 variations Small/Medium/Large with Medium isDefault) + 4 functional: add → POST `/variations` with nameKa + absolute price (base+modifier) + DB row written via `prismaTest.productVariation.findMany`, default radio → PUT `/variations/[vid]` + DB invariant (exactly 1 default, and it's Large), drag Small past Large → POST `/variations/reorder` + DB order flips to Medium/Large/Small, helper text contains `basePrice.toFixed(2) + ₾`. `pnpm db:push` green, `tsc --noEmit --skipLibCheck` clean on touched files, `next lint` clean, all 3 admin.json files parse valid, Playwright list enumerated 10 tests. Phase 14 now 2/6 (33%); overall 76/112 = 68% (+1 from T14.3). WIP=5 (T14.2, T14.4, T15.6, T15.13, T18.2). Visual baseline needs first-run `pnpm test:e2e:update`.)
...
🔓 **Still Blocked in Phase 14** (gated on IN_PROGRESS tasks): T14.5 Allergens STARTER locked gated on T14.4 🔄; T14.6 Error+Saving gated on T14.2 🔄.
🔜 **Next in Phase 14** (gated on T14.2/T14.4): T14.5 Allergens STARTER locked (deps T14.4), T14.6 Error + Saving (deps T14.2).
...
🔓 **Still Unlocked** (Phase 15, deps satisfied by T13.1 ✅ + T10.4 ✅): T15.5 Date Range Picker, T15.6 FREE Locked + Empty States. (T15.13 🔄 in progress.)
...
Last Updated: 2026-04-24 (T14.3 done ✅ — Product Drawer · Variations Tab shipped; dnd-kit drag-drop 5-col table + inline Add-variation row (KA name + modifier Input, absolute price = base+modifier) + Default radio (transactional isDefault flip) + kebab→Delete; Prisma `ProductVariation.isDefault` added + pushed; `ProductDialog` gained `menuId` prop and swaps placeholder → real panel in edit mode; EN/KA/RU variationsTab keys added; Playwright spec 5 desktop tests (visual + add/default/reorder/helper functional); Phase 14 now 2/6 (33%); overall 76/112 = 68%; WIP=5 (T14.2, T14.4, T15.6, T15.13, T18.2))

...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Build where clause
    const where: {
      category: { menuId: string; isSystemOffers?: boolean };
      categoryId?: string;
      isAvailable?: boolean;
    } = {
      // T22.24 — generated combo products live in the system Offers category
      // and are managed via their promotion, not the product list.
      category: { menuId, isSystemOffers: false },
    };
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Count products across all categories in this menu. T22.24 — combo
    // products are generated by the combo flow into the system Offers category,
    // so they must not burn the owner's plan allowance.
